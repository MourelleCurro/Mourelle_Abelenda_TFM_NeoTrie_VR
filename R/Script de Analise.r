# ==============================================================================
# SCRIPT DE ANÁLISE DE DATOS EMPREGADO NO TFM
# Autor: Ángel Mourelle Abelenda
# Titora: Teresa Fernández Blanco
# ==============================================================================
# Este script ten a finalidade de que o lector do TFM poida reproducir os datos
#     e mesmo modificalos para extraer novas gráficas e análises estatísticas
# ==============================================================================

# Aquí seleccionen a ruta da carpeta na que se atopa aloxado o script
#     e o arquivo de resultados
...
# ==============================================================================
# A continuación lístanse as librarías empregadas neste script.
# Compróbase se todos os paquetes están instalados, e en caso contrario
#   instálanse automaticamente.
# Logo cárganse todas as librarías necesarias antes de comezar
# ==============================================================================
...
library(forcats)     # Para ordenar as categorías nos gráficos
library(patchwork)   # Para combinar varios gráficos nunha soa imaxe
library(ggalluvial)  # Para crear gráficos de fluxo (aluviais)
library(ggcorrplot)  # Para crear matrices de correlación visuais (correlogramas)
...
#
# O CSV engade elementos ao final dos textos que se repiten (coma os items
#   dalgunhas preguntas). Esta función limpa esas etiquetas para as gráficas.
...
# Paleta de cores estándar para escalas Likert
...
#   puntos, etc.) e asígnanse nomes curtos ás columnas fundamentais.
# ==============================================================================
...
# Renomeado de variables principais segundo a súa posición para maior comodidade
...
# Número total de participantes (usarase nos cálculos de porcentaxes)
...
#       Preguntas 2, 3, 4 e 5
# ==============================================================================
# Xéranse os primeiros gráficos sobre o perfil das persoas enquisadas: 
#   o xénero, o ano de nacemento e a titulación de acceso previos.
# Finalmente, combínanse os tres gráficos nunha única imaxe
# ==============================================================================

# 4.1 Xénero (Gráfico de barras)
...
# ==============================================================================
# 5. EXPERIENCIA / COÑECEMENTO PREVIO E PERCEPCIÓN PREVIA DE HABILIDADES
#       Preguntas 6, 7, 8, 9 e 10
# ==============================================================================
# Trátase o coñecemento / uso previo da VR, a frecuencia de uso / consumo de
#     produtos e servizos relacionados, e a autopercepción (likert) de
#     habilidade en aspectos relacionados
# ==============================================================================

# 5.1 Coñecía / Empregou IVR (Barras agrupadas)
...
# 5.2 Tipo de experiencias con VR (Gráfico de sectores)
...
# 5.3 Habilidade Previa Percibida (Likert diverxente)
orde_habilidades <- c("Controis táctiles (smartphone, tablet...)", 
                      "Controladores / mandos de videoxogos", 
                      "Control por movemento (Wii, Kinect...)", 
                      "Visores de Realidade Virtual")

datos_habilidade <- datos %>%
  select("Controis táctiles (smartphone, tablet...)" = 13,
...
grafico_hab_diverxente <- ggplot() +
  ...
  labs(title = "Autopercepción previa da habilidade",
...
#       Preguntas 13 e 14
# ==============================================================================
# Dificultade percibida (likert) na reaización de cada unha das tarefas
#     da sesión.
# ==============================================================================
...
grafico_dificultade_act <- ggplot() +
  ...
  scale_fill_manual(values = cores_likert) +
  scale_y_continuous(labels = function(x) paste0(abs(x), "%"), limits = c(-100, 100), breaks = seq(-100, 100, by = 25)) +
  labs(title = "Facilidade/dificultade percibida das actividades realizadas",
       subtitle = "Esquerda: Dificultade (1-3) | Centro: (4) | Dereita: Facilidade (5-7)",
...
# ==============================================================================
# 7. ANÁLISE EN RETROSPECTIVA
#       Pares de preguntas 11 - 17 e 12 - 18
# ==============================================================================
# Estes gráficos constrastan o que pensaban os futuros docentes antes da sesión
#   fronte ao que opinan logo de probar a IVR, avaliando tanto a súa 
#   propia dificultade no uso (P11 vs P17) como a que experimentaría o alumnado
#   (P12 vs P18), mediante likerts de dobre barra
# ==============================================================================
...
grafico_evolucion <- ggplot() +
  ...
  scale_y_continuous(labels = function(x) paste0(abs(x), "%"), limits = c(-100, 100), breaks = seq(-100, 100, by = 25)) +
  labs(title = "Evolución da autopercepción da dificultade", subtitle = "Cores claras: Pre-sesión | Cores sólidas: Post-sesión", x = "", y = "Porcentaxe de respostas sobre o total do grupo", fill = "Valoración Likert (1 a 7)", alpha = "Momento") +
  theme_minimal() +
  theme(legend.position = "bottom", legend.box = "vertical", plot.title = element_text(face = "bold", hjust = 0.5, size = 14),
        plot.subtitle = element_text(hjust = 0.5, color = "gray40", size = 10, margin = margin(b = 10)),
        panel.grid.major.y = element_blank(), panel.grid.major.x = element_line(color = "gray90"),
        strip.text.y.left = element_text(angle = 0, face = "bold", size = 14, hjust = 1), 
...
grafico_evolucion_alum <- ggplot() +
  ...
  scale_y_continuous(labels = function(x) paste0(abs(x), "%"), limits = c(-100, 100), breaks = seq(-100, 100, by = 25)) +
  labs(title = "Evolución da dificultade percibida para o alumnado", subtitle = "Cores claras: Pre-sesión | Cores sólidas: Post-sesión", x = "", y = "Porcentaxe de respostas sobre o total do grupo", fill = "Valoración Likert (1 a 7)", alpha = "Momento") +
  theme_minimal() +
  theme(legend.position = "bottom", legend.box = "vertical", plot.title = element_text(face = "bold", hjust = 0.5, size = 14),
        plot.subtitle = element_text(hjust = 0.5, color = "gray40", size = 10, margin = margin(b = 10)),
        panel.grid.major.y = element_blank(), panel.grid.major.x = element_line(color = "gray90"),
...
#       Preguntas 19, 20 e 21
# ==============================================================================
# Confianza do futuro docente na súa capacidade para implementar
#   de forma a IVR no ensino (manexo de hardware/software,
#   deseño de aulas, etc.) 
# Visualízase en que sentidos matemáticos 
...
# 8.2 Sentidos Matemáticos (P20 e P21)
...
# ==============================================================================
# 9. MÉTODOS E ORGANIZACIÓN DA AULA
#     Preguntas 23 e 24
# ==============================================================================
# Compatibilidade entre métodos de ensino e as IVR, e compatibilidade persoal
#   do docente con cada un dos métodos, mediante un likert de dobre barra
# ==============================================================================
...
# ==============================================================================
# 10. APLICACIÓN NA AULA
#       Preguntas 26 e 28
# ==============================================================================
# Cantidade de sesións a adicar ás IVR en cada curso da ESO
# Proporción a aplicar de tres formas de organización dos dispositivos e a aula
# ==============================================================================
...
#       Preguntas 15, 16 e 22
# ==============================================================================
# Gráficas destas preguntas cualitativas, onde se agrupan as respostas
#     por categorías
# ==============================================================================
...
# ==============================================================================
# 12. ANÁLISE CUALITATIVA: DIFICULTADES
#         Preguntas 25 e 27
# ==============================================================================
# Pregunta de resposta aberta (agrúpanse por categorías) e pregunta de ordenar 
#     items (recóllense a porcentaxe de persoas que ubicaron cada item en 
#     cada posto)
# ==============================================================================
...
# ==============================================================================
# 13. PROPOSTAS PREGUNTA ABERTA E CORRELOGRAMA DE EXEMPLO
#         Pregunta 29 e exemplo de correlograma
# ==============================================================================
# Gráfico aluvial sobre a pregunta 29, onde se agrupan os cursos, bloques de 
#     contidos e actividades en categorías de resposta.
# Exemplo de correlograma, relacionando o item
#     "Deseñar conceptualmente actividades novas para realizar en NeoTrie VR"
#      coas medias das respostas ás preguntas 10, 11, 14 e 17
# ==============================================================================
...
colnames(datos_indices_especifico) <- c("Deseño Conceptual (P19)", "Facilidade Post-sesión (P17)", "Facilidade nas Tarefas (P14)", "Facilidade Pre-sesión (P11)", "Habilidade Tecnolóxica (P10)")
